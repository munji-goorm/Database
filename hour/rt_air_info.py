...
if (len(body) != 0):
    ...
    # Cursor Object 가져오기
    curs = conn.cursor()
    sql = "CREATE TABLE IF NOT EXISTS rt_air_info(so2_value Float, pm10_value int, pm25_value int, mang_name varchar(15), station_name varchar(15) PRIMARY KEY, no2_value Float, khai_value int, co_value Float, sido_name varchar(15), data_time varchar(30), o3_value Float)"
    curs.execute(sql)
    ...
    for a in body:
        ...
        sidoName = a['sidoName']
        dataTime = a['dataTime']
        so2Value = a['so2Value'] if ((a['so2Flag'] is None) and (a['so2Value'] is not None)) else -1
        coValue = a['coValue'] if ((a['coFlag'] is None) and (a['coValue'] is not None)) else -1
        pm10Value = a['pm10Value'] if ((a['pm10Flag'] is None) and (a['pm10Value'] is not None)) else -1
        pm25Value = a['pm25Value'] if ((a['pm25Flag'] is None) and (a['pm25Value'] is not None)) else -1
        no2Value = a['no2Value'] if ((a['no2Flag'] is None) and (a['no2Value'] is not None)) else -1
        o3Value = a['o3Value'] if ((a['o3Flag'] is None) and (a['o3Value'] is not None)) else -1
